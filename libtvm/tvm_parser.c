...
const char *tvm_opcode_map[] = {"nop", "int",  "mov", "push", "pop", "pushf", "popf", "inc", "dec", "add", "sub",
                                "mul", "div",  "mod", "rem",  "not", "xor",   "or",   "and", "shl", "shr", "cmp",
                                "jmp", "call", "ret", "je",   "jne", "jg",    "jge",  "jl",  "jle", "prn", 0};
...
static int *
token_to_register(const char *token, struct tvm_mem *mem);
static int
instr_to_opcode(const char *instr);

int
tvm_parse_labels(struct tvm_ctx *vm, const char ***tokens)
{
    int num_instr = 0;
    ...
    for (int i = 0; tokens[i]; i++)
    {
        ...
        for (int token_idx = 0; token_idx < MAX_TOKENS; token_idx++)
        {
            ...
            *label_delimiter = 0;
            ...
            int label_addr = tvm_htab_find(p->label_htab, tokens[i][token_idx]);
            ...
            tvm_htab_add(p->label_htab, tokens[i][token_idx], num_instr);
        }
 ...
 */
static int **
tvm_parse_args(struct tvm_ctx *vm, const char **instr_tokens, int *instr_place)
{
    int **args = calloc(sizeof(int *), MAX_ARGS);

    for (int i = 0; i < MAX_ARGS; ++i)
    {
        if (!instr_tokens[*instr_place + 1 + i] || !strlen(instr_tokens[*instr_place + 1 + i]))
            continue;
        ...
        int *regp = token_to_register(instr_tokens[*instr_place + 1 + i], vm->mem);
        ...
        {
            char *end_symbol = strchr(instr_tokens[*instr_place + 1 + i], ']');

            if (end_symbol)
            {
                *end_symbol = 0;
                args[i] = &((int *)vm->mem->mem_space)[tvm_parse_value(instr_tokens[*instr_place + 1 + i] + 1)];
        ...
        int addr = tvm_htab_find(vm->prog->label_htab, instr_tokens[*instr_place + 1 + i]);
        ...
        args[i] = tvm_add_value(vm, tvm_parse_value(instr_tokens[*instr_place + 1 + i]));
    }
 ...
 */
static int
tvm_parse_instr(struct tvm_ctx *vm, const char **instr_tokens, int *instr_place)
{
    for (int token_idx = 0; token_idx < MAX_TOKENS; token_idx++)
    {
        /* Skip empty tokens */
...
int
tvm_parse_program(struct tvm_ctx *vm, const char ***tokens)
{
    int line_idx;
    ...
    for (line_idx = 0; tokens[line_idx]; line_idx++)
    {
        ...
        int opcode = tvm_parse_instr(vm, tokens[line_idx], &instr_place);

        if (opcode == -1)
            continue;

        int **args = tvm_parse_args(vm, tokens[line_idx], &instr_place);
        ...
        newptr = realloc(vm->prog->instr, sizeof(int) * (vm->prog->num_instr + 1));
        ...
        newptr = realloc(vm->prog->args, sizeof(int **) * (vm->prog->num_instr + 1));
...
int *
tvm_add_value(struct tvm_ctx *vm, const int val)
{
    struct tvm_prog *p = vm->prog;
...
int
tvm_parse_value(const char *str)
{
