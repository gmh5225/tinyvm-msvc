...
void
tvm_htab_destroy(struct tvm_htab_ctx *htab)
{
...
htab_hash(const char *k, const unsigned int size)
{
    unsigned int hash = 1;
...
static void
htab_rehash(struct tvm_htab_ctx *orig, unsigned int size)
{
    int i;
    struct tvm_htab_node *node, *next;
    ...
    for (i = 0; i < orig->size; i++)
    {
        ...
        while (node)
        {
            ...
            if (node->valptr)
            {
                tvm_htab_add_ref(new, node->key, node->valptr, strlen(node->valptr) + 1);
...
static struct tvm_htab_node *
htab_add_core(struct tvm_htab_ctx *htab, const char *k)
{
...
int
tvm_htab_add(struct tvm_htab_ctx *htab, const char *key, int value)
{
...
int
tvm_htab_add_ref(struct tvm_htab_ctx *htab, const char *key, const void *valptr, int len)
{
    struct tvm_htab_node *node = htab_add_core(htab, key);
...
static struct tvm_htab_node *
htab_find_core(struct tvm_htab_ctx *htab, const char *key)
{
...
int
tvm_htab_find(struct tvm_htab_ctx *htab, const char *key)
{
...
char *
tvm_htab_find_ref(struct tvm_htab_ctx *htab, const char *key)
{
    struct tvm_htab_node *node = htab_find_core(htab, key);
