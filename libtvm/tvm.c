...
tvm_vm_create()
{
    struct tvm_ctx *vm = (struct tvm_ctx *)calloc(1, sizeof(struct tvm_ctx));
...
int
tvm_vm_interpret(struct tvm_ctx *vm, char *filename)
{
    FILE *filp = NULL;
    int source_length = 0;
...
void
tvm_vm_destroy(struct tvm_ctx *vm)
{
    if (vm && vm->mem)
